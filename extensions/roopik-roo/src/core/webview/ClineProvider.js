...
export class ClineProvider extends EventEmitter {
    ...
    settingsImportedAt;
    latestAnnouncementId = "feb-2026-v3.50.0-gemini-31-pro-cli-ndjson-cli-v010"; // v3.50.0 Gemini 3.1 Pro Support, CLI NDJSON Protocol, CLI v0.1.0
    ...
    */
    ...
    async createTaskWithHistoryItem(historyItem, options) {
        ...
        if (historyItem.apiConfigName && !skipProfileRestoreFromHistory) {
            ...
            this.log(`Skipping restore of provider profile '${historyItem.apiConfigName}' for task ${historyItem.id} in CLI runtime.`);
        }
    ...
    async getTaskWithId(id) {
        const historyItem = this.taskHistoryStore.get(id) ?? (this.getGlobalState("taskHistory") ?? []).find((item) => item.id === id);
        if (!historyItem) {
    ...
    async getStateToPostToWebview() {
        ...
        return {
            ...
            alwaysAllowRoopik: alwaysAllowRoopik ?? false,
    ...
    async getState() {
        ...
        return {
            ...
            alwaysAllowRoopik: stateValues.alwaysAllowRoopik ?? false,
    ...
    async createTask(text, images, parentTask, options = {}, configuration = {}) {
        ...
        const { apiConfiguration, organizationAllowList, enableCheckpoints, checkpointTimeout, experiments } = await this.getState();
