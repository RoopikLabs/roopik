...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	return path.join(cwd, ".dio", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .dio/system-prompt-[mode slug]
...
/**
 * Ensures the .dio directory exists, creating it if necessary
 */
export async function ensureRooDirectory(cwd: string): Promise<void> {
	const rooDir = path.join(cwd, ".dio")
