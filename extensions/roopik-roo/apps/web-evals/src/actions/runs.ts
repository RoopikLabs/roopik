...
export async function createRun({
	...
	try {
		...
		// If execution method is CLI, run directly without Docker
		if (executionMethod === "cli") {
			console.log("spawn ->", cliCommand)

			const childProcess = spawn(cliCommand, {
				detached: true,
				stdio: ["ignore", "pipe", "pipe"],
				shell: true, // Use shell to handle pnpm command on Windows
			})

			const logPath = path.join(os.tmpdir(), "roo-code-evals.log")
			...
			// VSCode execution method uses Docker
