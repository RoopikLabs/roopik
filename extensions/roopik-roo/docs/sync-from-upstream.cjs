#!/usr/bin/env node
/**
 * Sync From Upstream Tool
 *
 * Downloads changed files from specific commits in the upstream Roo Code repo
 * and copies them to the correct location in your roopik repo.
 *
 * Usage:
 *   node sync-from-upstream.cjs <commit-or-range> [options]
 *
 * Examples:
 *   # Dry run (just list files, don't download)
 *   node sync-from-upstream.cjs abc1234 --dry-run
 *   node sync-from-upstream.cjs --last 5 --dry-run
 *
 ...
 *   node sync-from-upstream.cjs abc1234 --preview
 *
 *   # Stage files for manual review (keeps original folder structure)
 *   node sync-from-upstream.cjs abc1234 --stage
 *
 * IMPORTANT NOTES for Range Commits:
 *   When using ranges (B..Z), ALWAYS use your LAST successfully synced commit as the start.
 *
 *   CORRECT:   node sync-from-upstream.cjs B..Z  (where B is your last sync)
 *   WRONG:     node sync-from-upstream.cjs C..Z  (you'll miss changes from commit C)
 *
...
// Configuration
const CONFIG = {
	// Upstream Roo Code repo
	upstreamOwner: 'RooCodeInc',
	upstreamRepo: 'Roo-Code',
	upstreamBranch: 'main',
...
async function syncFromUpstream(commitSpec, options = {}) {
	...
	log('  Sync From Upstream Roo Code', colors.bright + colors.cyan);
	log('='.repeat(60) + '\n', colors.bright);
	...
	const modeLabel = stage ? 'STAGING (original folder structure)' : 'DIRECT SYNC (mapped paths)';
...
function showHelp() {
	console.log(`
${colors.bright}Sync From Upstream Tool${colors.reset}

Downloads changed files from specific commits in the upstream Roo Code repo
and copies them to the correct location in your roopik repo.

${colors.bright}Usage:${colors.reset}
  node sync-from-upstream.js <commit-or-range> [options]
...
${colors.bright}Examples:${colors.reset}
  # Sync a single commit
  node sync-from-upstream.js abc1234

  # Sync a range of commits
  node sync-from-upstream.js abc1234..def5678

  # Sync last 5 commits
  node sync-from-upstream.js --last 5

  # Dry run to see what would change
  node sync-from-upstream.js abc1234 --dry-run

  # Stage files for manual review (keeps original folder structure)
  node sync-from-upstream.js abc1234 --stage
  # Files will be in .upstream-sync/ folder - copy them to extensions/roopik-roo/
