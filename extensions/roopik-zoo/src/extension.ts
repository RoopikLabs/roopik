...
async function checkWorktreeAutoOpen(
	...
	try {
		...
		if (normalizePath(currentPath) === normalizePath(worktreeAutoOpenPath)) {
			...
			setTimeout(async () => {
				try {
					await vscode.commands.executeCommand("roopik-zoo.plusButtonClicked")
				} catch (error) {
...
export async function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.window.registerWebviewViewProvider("roopik-zoo.ChatPanel", chatPanelProvider, {
			webviewOptions: { retainContextWhenHidden: true },
		}),
	)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("roopik-zoo.openChatPanel", async (options?: { message?: string; code?: string }) => {
			// Focus the chat panel view (this will automatically show the auxiliary bar)
			await vscode.commands.executeCommand("roopik-zoo.ChatPanel.focus")
