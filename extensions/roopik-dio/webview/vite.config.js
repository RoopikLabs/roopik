...
export default defineConfig(({ mode }) => {
    // ROOPIK: Changed output to webview/build (not src/webview-ui/build)
    let outDir = "./build";
    ...
    const define = {
        ...
        "process.env.PKG_OUTPUT_CHANNEL": JSON.stringify("Roopik-Dio"),
    ...
    if (mode === "nightly") {
        ...
        const nightlyPkg = JSON.parse(fs.readFileSync(path.join(__dirname, "..", "apps", "vscode-nightly", "package.nightly.json"), "utf8"));
    ...
    const plugins = [react(), tailwindcss(), persistPortPlugin(), wasmPlugin(), sourcemapPlugin()];
    return {
        plugins,
        resolve: {
            alias: {
