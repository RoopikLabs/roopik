...
describe("Model Validation Functions", () => {
    ...
    const allowAllOrganization = {
        allowAll: true,
        providers: {},
    };
    const restrictiveOrganization = {
        allowAll: false,
        providers: {
            openrouter: {
                allowAll: false,
    ...
    describe("validateApiConfigurationExcludingModelErrors", () => {
        ...
        it("excludes model-specific organization errors", () => {
            ...
            const result = validateApiConfigurationExcludingModelErrors(config, mockRouterModels, restrictiveOrganization);
...
describe("validateBedrockArn", () => {
    describe("always returns isValid: true (no strict format validation)", () => {
        it("accepts standard AWS Bedrock ARNs", () => {
            const result = validateBedrockArn("arn:aws:bedrock:us-west-2:123456789012:inference-profile/us.anthropic.claude-3-5-sonnet-v2");
            expect(result.isValid).toBe(true);
            expect(result.arnRegion).toBe("us-west-2");
            expect(result.errorMessage).toBeUndefined();
        });
        it("accepts AWS GovCloud ARNs", () => {
            const result = validateBedrockArn("arn:aws-us-gov:bedrock:us-gov-west-1:123456789012:inference-profile/model");
            expect(result.isValid).toBe(true);
    ...
    describe("region mismatch warnings", () => {
        it("shows warning when ARN region differs from provided region", () => {
            const result = validateBedrockArn("arn:aws:bedrock:us-west-2:123456789012:inference-profile/model", "us-east-1");
        ...
        it("shows no warning when ARN region matches provided region", () => {
            const result = validateBedrockArn("arn:aws:bedrock:us-west-2:123456789012:inference-profile/model", "us-west-2");
            expect(result.isValid).toBe(true);
            expect(result.arnRegion).toBe("us-west-2");
            expect(result.errorMessage).toBeUndefined();
        });
