import { insertMention, removeMention, getContextMenuOptions, shouldShowContextMenu, ContextMenuOptionType, } from "@src/utils/context-mentions";
...
describe("getContextMenuOptions", () => {
    ...
    it("should maintain correct result ordering according to implementation", () => {
        ...
        const fileResults = result.filter((item) => item.type === ContextMenuOptionType.File ||
            item.type === ContextMenuOptionType.OpenedFile ||
            item.type === ContextMenuOptionType.Folder);
        const searchResults = result.filter((item) => item.type === ContextMenuOptionType.File && item.value?.includes("/search/"));
    ...
    it("should deduplicate items correctly when combining different result types", () => {
        ...
        const duplicateCount = result.filter((item) => (item.value === "src/test.ts" || item.value === "/src/test.ts") &&
            item.type === ContextMenuOptionType.File).length;
    ...
    it("should return NoResults when all combined results are empty with dynamic search", () => {
        ...
        const result = getContextMenuOptions("nonexistentquery123456", null, mockQueryItems, []);
        expect(result).toHaveLength(1);
        expect(result[0].type).toBe(ContextMenuOptionType.NoResults);
    });
    /**
    ...
    it("should place opened files first in result order", () => {
        ...
        const firstSearchResultIndex = result.findIndex((item) => item.type === ContextMenuOptionType.File && item.value?.includes("/search/"));
