...
beforeEach(() => {
    vi.clearAllMocks();
    getHighlighter.mockResolvedValue(mockHighlighter);
});
describe("highlightHunks", () => {
    it("should highlight simple old and new text", async () => {
        const result = await highlightHunks("const x = 1\nconsole.log(x)", "const x = 2\nconsole.log(x)", "javascript", "light");
    ...
    it("should handle single-line text", async () => {
        ...
        expect(mockHighlighter.codeToHast).toHaveBeenCalledWith("const x = 1", expect.objectContaining({
            lang: "javascript",
            theme: "github-dark",
        }));
    });
    ...
    it("should map light theme to github-light", async () => {
        ...
        expect(mockHighlighter.codeToHast).toHaveBeenCalledWith("test", expect.objectContaining({
            theme: "github-light",
        }));
    });
    ...
    it("should handle getHighlighter rejection", async () => {
        ;
        getHighlighter.mockRejectedValueOnce(new Error("Highlighter failed"));
        const result = await highlightHunks("const x = 1", "const x = 2", "javascript", "light");
        // Should fall back to plain text
        expect(result.oldLines).toEqual(["const x = 1"]);
        expect(result.newLines).toEqual(["const x = 2"]);
...
//# sourceMappingURL=highlightDiff.spec.js.map