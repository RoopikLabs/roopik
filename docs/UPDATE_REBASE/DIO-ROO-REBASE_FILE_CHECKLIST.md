...
## 5. Adding a New Tool — Files to Update

When adding a new browser/project/canvas/component tool, update **all** of these files:

| # | File Path | What to Add |
|---|-----------|-------------|
| 1 | `packages/types/src/tool.ts` | Tool name in `ToolName` array |
| 2 | `packages/types/src/vscode-extension-host.ts` | Tool name in `ClineSayTool` union type |
| 3 | `src/core/prompts/tools/native-tools/roopik.ts` | OpenAI schema export + add to `roopikNativeTools` array |
| 4 | `src/core/prompts/tools/roopik/roopik-tools.ts` | Tool name in `ROOPIK_TOOL_NAMES` array |
| 5 | `src/shared/tools.ts` | `NativeToolArgs` type + `TOOL_DISPLAY_NAMES` + `TOOL_GROUPS.roopik` |
| 6 | `src/core/assistant-message/NativeToolCallParser.ts` | Streaming partial + final extraction switch cases |
| 7 | `src/core/assistant-message/presentAssistantMessage.ts` | Display case + routing switch case |
| 8 | `src/core/tools/roopik/RoopikToolHandler.ts` | Switch case + handler function |
| 9 | `src/services/roopik/RoopikToolClient.ts` | Client method calling `executeCommand` |
| 10 | `webview-ui/src/components/chat/ChatRow.tsx` | UI render case with icon |
| 11 | `webview-ui/src/i18n/locales/en/chat.json` | `wantsTo...` / `did...` translations |

> Also update tool count comments (e.g., "Browser (16)") wherever they appear.

### Native MCP / Core IDE (outside `extensions/roopik-roo/`)

**Base Path:** `src/vs/workbench/contrib/roopik/`

| # | File Path | What to Add |
|---|-----------|-------------|
| 1 | `electron-main/mcp/toolSchemas.ts` | Zod schema + entry in `TOOL_DEFINITIONS` array |
| 2 | `electron-main/mcp/executor/types.ts` | Result type interface (if tool returns new fields) |
| 3 | `electron-main/mcp/executor/toolExecutor.ts` | Tool name in list + switch case |
| 4 | `electron-main/channel/roopikToolsChannel.ts` | Switch case + handler delegation |
| 5 | `electron-main/tools/browserToolService.ts` | Service method implementation |
| 6 | `electron-main/mcp/prompts/mcpPrompts.ts` | Tool category listing |
| 7 | `browser/commands/roopikToolsCommands.ts` | `registerAction2` command registration |

**STDIO Binary (auto-synced):**

| # | File Path | What to Add |
|---|-----------|-------------|
| 1 | `tools/roopik/mcp-stdio-server/src/tools.ts` | Run `npm run sync-schemas` (copies from toolSchemas.ts) |
| 2 | Same file — `MCP_INSTRUCTIONS` prompt section | Manually update tool categories + total count |

> After syncing, rebuild: `npm run build:binary:win`
