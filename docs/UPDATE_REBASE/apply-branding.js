...
// Apply build/next/index.ts updates - Add Roopik resource patterns to esbuild pipeline
function updateBuildNextIndex() {
	const filePath = path.join(ROOT_DIR, 'build/next/index.ts');

	if (!fileExists(filePath)) {
		warning('build/next/index.ts not found (skipping - old build pipeline without esbuild)');
		return { updated: false, errors: 0 };
	}
	...
	// Resource entries needed in desktopResourcePatterns (without the out-build/ prefix)
	const roopikEntries = [
	...
	if (missingEntries.length === 0) {
		success('build/next/index.ts - Roopik resource entries already present');
		return { updated: false, errors: 0 };
	}

	// Find desktopResourcePatterns array
	const arrayMarker = 'const desktopResourcePatterns = [';
	const arrayStart = content.indexOf(arrayMarker);
	if (arrayStart === -1) {
		warning('build/next/index.ts - Could not find desktopResourcePatterns array');
		return { updated: false, errors: 0 };
	}
	...
	const arrayEnd = content.indexOf('];', arrayStart);
	if (arrayEnd === -1) {
		warning('build/next/index.ts - Could not find end of desktopResourcePatterns array');
		return { updated: false, errors: 0 };
	}

	// Build the Roopik block
	const roopikBlock = [
		'',
		'\t// Roopik',
	...
	const updatedContent = content.slice(0, arrayEnd) + roopikBlock + '\n' + content.slice(arrayEnd);

	if (writeFile(filePath, updatedContent)) {
		success(`build/next/index.ts - Added ${missingEntries.length} Roopik resource entry(s) to desktopResourcePatterns`);
		return { updated: true, errors: 0 };
	}

	error('build/next/index.ts - Failed to update');
	return { updated: false, errors: 1 };
}
...
function main() {
	...
	const buildNextResult = updateBuildNextIndex();
	if (buildNextResult.updated) {
		totalChanges++;
	}
	totalErrors += buildNextResult.errors;
