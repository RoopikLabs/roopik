...
## Build Pipeline Transition: gulp-tsb → esbuild (upstream)

**Status:** In-progress upstream migration. Both pipelines coexist, controlled by a feature flag.

**Toggle:** `build/buildConfig.ts` → `useEsbuildTranspile` (currently `true`)

| Pipeline | Config File | Resource Array | Gulp Task |
|----------|-------------|----------------|-----------|
| **Old** (gulp-tsb) | `build/gulpfile.vscode.ts` | `vscodeResourceIncludes` | `core-ci-old` |
| **New** (esbuild) | `build/next/index.ts` | `desktopResourcePatterns` | `core-ci-esbuild` |

**Selection logic** (`gulpfile.vscode.ts`):
```typescript
gulp.task(task.define('core-ci', useEsbuildTranspile ? coreCIEsbuild : coreCIOld));
```

**Why two?** Microsoft is migrating VS Code's build from gulp-tsb (TypeScript compiler via gulp) to esbuild (faster transpilation). During the transition, both pipelines exist — the old one renamed to `core-ci-old`, the new one as `core-ci-esbuild`. The flag selects which one runs as `core-ci`.

**Roopik impact:** Both pipelines have their own curated list of resource files (media, images, JSON) to copy during production builds. Roopik entries must be added to BOTH:
1. `vscodeResourceIncludes` in `gulpfile.vscode.ts` (prefixed with `out-build/`)
2. `desktopResourcePatterns` in `build/next/index.ts` (no prefix)

**Automation:** `apply-branding.js` handles both via `updateGulpfileVscode()` and `updateBuildNextIndex()`.

**Per-target resource patterns** (esbuild pipeline only):

| Array | Target | Has Roopik? |
|-------|--------|-------------|
| `commonResourcePatterns` | All targets | No (only SVGs from CSS, tree-sitter) |
| `desktopResourcePatterns` | Electron desktop | **Yes** — welcome logo, Dio icons, resources, devServer |
| `serverResourcePatterns` | Server (REH) | No (shell scripts only, no UI) |
| `serverWebResourcePatterns` | Server + Web UI | No (Roopik is desktop-only) |
| `webResourcePatterns` | Browser-only | No (Roopik is desktop-only) |

**Note:** Once Microsoft completes the migration and removes the old pipeline, we can drop `updateGulpfileVscode()` from the automation and the `vscodeResourceIncludes` entries. Until then, maintain both.
