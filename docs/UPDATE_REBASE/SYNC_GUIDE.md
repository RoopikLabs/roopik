...
## ⚠️ Note: Dual Build Pipeline (Resource Patterns)

Upstream VS Code is transitioning from **gulp-tsb** to **esbuild** for builds. Both pipelines coexist, controlled by `build/buildConfig.ts` → `useEsbuildTranspile` (currently `true`).

Roopik media files (welcome logo, Dio icons, resources) must be registered in **both** pipelines' resource lists:

| Pipeline | File | Array |
|----------|------|-------|
| Old (gulp-tsb) | `build/gulpfile.vscode.ts` | `vscodeResourceIncludes` |
| New (esbuild) | `build/next/index.ts` | `desktopResourcePatterns` |

`apply-branding.js` handles both automatically (`updateGulpfileVscode()` + `updateBuildNextIndex()`). If images are missing from the final build, check that both arrays include the Roopik entries. See `VSCODE_MODIFICATIONS.md` → "Build Pipeline Transition" for full details.
